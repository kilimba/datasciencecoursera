...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Getting Started", tabName = "widgets", icon = icon("file-powerpoint-o"))
    ),
    tags$br(),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "widgets",
              ...
              infoBox(title = "Getting Started", width = 12,value = "Selecting the Age Group drop down menu will display the 
                      disease trend over time for that age group in the line chart (top right quadrant), disaggregated by 
                      gender. Selecting on the Disease Outcome dropdown menu selects that disease as the focus of the 
                      visualization on all 3 panels. Unchecking the Animate Pyramid check box renders a dropdown menu so that 
                      the user can select a specific year to be displayed on the population pyramid (top left quadrant)")
      )
    )
  )
)
